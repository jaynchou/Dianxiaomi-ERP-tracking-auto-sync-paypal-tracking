...
这是一个独立编写的 WooCommerce 插件：读取店小秘写入的运单号，通过 **WooCommerce PayPal Payments 自带的 Package Tracking 接口**回传给 PayPal，不保存另一套 PayPal Client ID 或 Secret。

## 主要功能

- 支持店小秘 API 订单备注、店小秘插件字段和 WooCommerce Shipment Tracking 字段。
- 仅同步 `Completed`（已完成发货）的订单，完全排除 `Processing`。
